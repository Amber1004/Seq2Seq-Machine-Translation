...
def trainIters(encoder, decoder, epochs, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    plt.ylabel("loss")
    plt.legend()
    plt.savefig("20epochs/Q6_Loss_scheculer2.png")
...
hidden_size = 512
encoder5 = TransformerEncoder(input_lang.n_words, hidden_size).to(device)
decoder5 = Decoder(hidden_size, output_lang.n_words).to(device)


print('Q6')
trainIters(encoder5, decoder5, 20, print_every=5000)
evaluateRandomly(encoder5, decoder5)
input,gt,predict,score = test(encoder5, decoder5, train_pairs)
input,gt,predict,score = test(encoder5, decoder5, test_pairs)
